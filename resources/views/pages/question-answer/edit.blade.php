        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                @if (session()->has('error'))
                    ...
                    @endphp
                @endif
                <form action="{{ route('admin.qa.update', $item) }}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-12">
                            ...
                            <div class="form-group mb-8">
                                ...
                                <div class="form-check form-check-inline mt-2">
                                    <input class="form-check-input" type="radio" name="status" id="inlineRadio1" value="1" {{ $item->status == 1 ? 'checked' : '' }}>
                                    <label class="form-check-label" for="inlineRadio1">Hoạt động</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input class="form-check-input" type="radio" name="status" id="inlineRadio2" value="2" {{ $item->status == 2 ? 'checked' : '' }}>
                                    <label class="form-check-label" for="inlineRadio2">Đóng</label>
                                </div>
                            </div>
                            <div class="form-group mb-8">
                                ...
                                <select placeholder="Chọn" class="form-select mb-2 select2-hidden-accessible" data-control="select2" data-hide-search="false" tabindex="-1" aria-hidden="true" name="category_id" onchange="$('#checkcategoryid').text('')">
                                    <option value="1" {{$item->category_id == 1 ? 'selected' : ''}}>Thực tập</option>
                                    <option value="2" {{$item->category_id == 2 ? 'selected' : ''}}>Việc làm</option>
                                    <option value="3" {{$item->category_id == 3 ? 'selected' : ''}}>Sự kiện</option>
                                </select>
                                @error('category_id')
                            ...
                            <div class="form-group mb-10">
                                ...
                                <input type="text" name="question" value="{{$item->question}}" class=" form-control"
                            ...
                            <div class="form-group mb-10">
                                ...
                                <textarea class="form-control" name="answer" id="kt_docs_ckeditor_classic" rows="3" onkeyup="$('#checkanswer').text('')">{{ $item->answer }}</textarea>
                                @error('answer')
